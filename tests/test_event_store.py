...
class TestAttachIdentity:
    def test_attaches_to_recent_person(self, store: EventStore, person_event: dict, monkeypatch):
        ...
        saved = json.loads(
            (store.events_root / "person/2026/06/06" / f"{event_id}.json").read_text()
        )
        assert saved["identity"]["name"] == "Thomas"
        assert saved["identity"]["confidence"] == 0.92
        assert saved["summary"] == "Thomas at front"

        timeline = json.loads(store.timeline_jsonl.read_text().strip())
        assert timeline["identity"]["name"] == "Thomas"

    def test_skips_wrong_camera(self, store: EventStore, person_event: dict):
        person_event["summary"] = make_summary(person_event)
        store.write(person_event)
        assert store.attach_identity("driveway_id", "thomas", 0.9) is None

    def test_skips_stale_events(self, store: EventStore, monkeypatch):
        old_ts = (datetime.now(TZ) - timedelta(minutes=5)).isoformat()
        event = {
            "timestamp": old_ts,
            "type": "person",
            "location": {"zone": "front", "camera": "front"},
        ...
        store.write(event)
        assert store.attach_identity("front", "thomas", 0.9) is None
